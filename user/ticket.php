...
if(isset($_POST['submit'])) {
    $message = '';
  if ($_POST['other-department'] == '' &&  $_POST['location'] == ''){
    $insert_sql = "INSERT INTO user_reservation_vehicle(user_id, title, template_name, departure, arrival, email, purpose_description, no_passengers, department_id, other_department, location_area, location)
      VALUES (
       ?, ?, ? , ?, ?, ? , ?, ?, ?, NULL, ?, NULL
      )";
  }

    elseif($_POST['other-department'] == ''){
    $insert_sql = "INSERT INTO user_reservation_vehicle(user_id, title, template_name, departure, arrival, email,  purpose_description, no_passengers, department_id, other_department, location_area, location)
      VALUES (
       ?, ?, ? , ? , ?, ?, ?, ?, ?, NULL, ?, ?
      )";
    ...
    $insert_sql = "INSERT INTO user_reservation_vehicle(user_id, title, template_name ,departure, arrival, email,  purpose_description, no_passengers, department_id, other_department, location_area, location)
      VALUES (
       ?, ? , ? , ?, ? , ?, ?, ?, ?, ? , ?, NULL
        ...
        $insert_sql = "INSERT INTO user_reservation_vehicle
                (user_id, title, template_name, departure, arrival, email,  purpose_description, no_passengers, department_id, other_department, location_area, location)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  try{
      ...
      if ($_POST['other-department'] == '' &&  $_POST['location'] == ''){
        $stmt->bind_param("isssssssiii", $user, $title, $template_name ,$departure_datetime, $arrival_datetime, $email, $purpose, $no_passengers, $department_id, $location_area_type, $template_name);
      }
    elseif ($_POST['other-department'] == '') {
    $stmt->bind_param("issssssiiis", $user, $title, $template_name ,$departure_datetime, $arrival_datetime, $email, $purpose, $no_passengers, $department_id, $location_area_type, $location);
    } elseif ($_POST['location'] == '') {
        $stmt->bind_param("issssssiisi", $user, $title, $template_name ,$departure_datetime, $arrival_datetime, $email, $purpose, $no_passengers, $department_id, $other_department, $location_area_type);
    } else {
        $stmt->bind_param("issssssiissi", $user, $title, $template_name ,$departure_datetime, $arrival_datetime, $email, $purpose, $no_passengers, $department_id, $other_department, $location_area_type, $location);
    }
      ...
      if($template_name == ''){
        $errors['template_name'] = 'Please provide a template_name';
      }
      if($email == ''){
        $errors['email'] = 'Please provide a title of travel';
      }
      if($date_departure == ''){
...
if (isset($_FILES['template_name']) && $_FILES['template_name']['error'] === UPLOAD_ERR_OK) {
    $tempFile = $_FILES['template_name']['tmp_name'];
    $targetPath = "upload/"; // Change this to your desired directory
    $template_name = $_FILES['template_name']['name'];
    $targetFile = $targetPath . $template_name;

    // Move the uploaded file to the target directory
    if (move_uploaded_file($tempFile, $targetFile)) {
        // File moved successfully
        $template_name = $targetFile; // Update template_name with the path
    } else {
        // Failed to move file
        $template_name = ''; // Set empty value if file upload failed
    }
} else {
...
<div class="page-wrapper">
    ...
    <?php endif?>
        ...
        <div class="card-header d-flex justify-content-between">
            ...
            <div class="col-12">
                <form method="post" enctype="multipart/form-data" class="row g-3"onsubmit="return validateTripTicketForm()">
                    <!---Reservation Time-->
                ...
                <div class="col-md-6">
                  ...
                  <button type="button" class="btn btn-primary mt-2" onclick="addPassenger()">Add Passenger</button>
                  <button type="button" class="btn btn-danger mt-2" onclick="deleteSelected()">Delete Selected</button>
...
<script>
  ...
  // Disable past dates in date input fields
  $(document).ready(function() {
    $('#date_departure').attr('min', currentDate);
    $('#date_arrival').attr('min', currentDate);
  });
</script>

<script>
    function addPassenger() {
        var inputElement = document.getElementById("passengerNames");
        var passengerName = inputElement.value.trim();

        if (passengerName !== "") {
            var listElement = document.getElementById("passengerList");
    ...
    function deleteSelected() {
        var listElement = document.getElementById("passengerList");
        var selectedItems = document.querySelectorAll("#passengerList li.active");
    ...
    // Add event listener to toggle the 'active' class on list items
    document.getElementById("passengerList").addEventListener("click", function (event) {
